...
export class ActionableInsightsComponent implements OnInit {
  ...
  private readonly ML_API_ENDPOINT = 'YOUR_ML_API_ENDPOINT'; // Replace with your actual API endpoint
  ...
  async sendMessage() {
    ...
    try {
      // Call ML API
      const response = await this.fetchClient.fetch(this.ML_API_ENDPOINT, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      this.chatMessages.push({
        ...
        html: this.convertMarkdownToHtml(data.response)
      });
    } catch (error) {
      // Remove loading message
      this.chatMessages.pop();
